...
class Gui:
    def __init__(self):
        ...
        if devMode:
            ...
            btn_pics = tk.Button(master=self.frame_base, text='Choose directory', width=12, height=1, font=font_buttons,
                                 bg=custom_orange,
            ...
            dir_text_box = tk.Label(master=self.frame_base, textvariable=self.dirVar, font=("Calibri", 10), bg=syscolor,
        ...
        self.btn_pics = tk.Button(master=self.frame_base, text='Choose manually', width=12, height=1, font=font_buttons,
                                  bg=custom_orange,
        ...
        repeat_button = tk.Button(master=self.frame_base, image=photo_repeat_button, bg=syscolor, bd=0,
                                  activebackground=syscolor, highlightthickness=0, command=self.repeat_pressed)
        ...
        detected_meals_lable = tk.Label(master=self.frame_base, text="Detected meals:", font=("Calibri", 16),
                                        bg=syscolor, foreground="white", width=25)
        detected_meals_lable.place(relx=0.72, rely=0.25, anchor=tk.NW)

        self.dirVarMoney = StringVar()
        self.dirVarMoney.set("Total: 0.00 €")
        dir_text_box_price = tk.Label(master=self.frame_base, textvariable=self.dirVarMoney, font=("Calibri", 20),
                                      bg=syscolor, foreground="white", width=25)
        dir_text_box_price.place(relx=0.683, rely=0.62, anchor=tk.NW)
        self.student_discount_lable = tk.Label(master=self.frame_base, text="Student-Discount applied!",
                                               font=("Calibri", 10), bg=syscolor, foreground="#00A912", width=25)
        ...
        self.cropdistX = 150
        self.cropdistY = 40
        self.image_label = tk.Label(self.frame_base, height=self.cam_height - 2 * self.cropdistY,
                                    width=self.cam_width - 2 * self.cropdistX, background=syscolor)
    ...
    def choose_directory(self):
        self.index = 0
        path = filedialog.askdirectory(initialdir="/home/jetson/desktop/cantinaSelfCheckout",
    ...
    def start(self, cam_number=0):
        ...
        while True:
            ...
            if ret:
                ...
                frame = frame[self.cropdistY:self.cam_height - self.cropdistY,
                        self.cropdistX:self.cam_width - self.cropdistX]
                    ...
                    if ret_val:
                        if not self.stop_detecting:
                            ...
                            if self.stop_detecting:
                                frame = cv2.putText(frame, "Detected", (frame.shape[0] - 40, 40),
                                                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                            else:
                                frame = cv2.putText(frame, detecting_string, (frame.shape[0] - 40, 40),
