...
def calibrateCamera(cap,rows,columns,squareSize,runs,saveImages = False, webcam = True):
    ...
    directory2 = "C:\\Users\\Lars\\Desktop\\TestBilder\\Nachher"   #Used visually to check if the corners of the
                                                                   #chessboard are detectet correctly
    ...
    for r in range(runs):       #for every calibration run
        ...
        if webcam:#Read in images from webcam
            ...
            if not os.path.exists("CalibrationImages/Run"+str(r+1)):
    ...
    #while True:    # plt.show creates problems !
     #   if cv2.waitKey(1) & 0xff == ord('x'):
     #       break
     #   plt.show()
...
if __name__ == '__main__':
    ...
    rows = 17  # 17   6
    columns = 28  # 28    9
    squareSize = 10  # mm      Beinflusst aber in keiner weise die Matrix
    calibrateCamera(video_capture, rows, columns, squareSize, 1, saveImages=False, webcam=True)
